  ...
  async function loadProfile() {
    try {
      ...
      const { data: profileData, error: profileError } = await supabase
        .from('organizers')
        .select('*')
        .eq('organizer_username', params.username)
        .single();
      ...
      <header class="profile-header" in:fly={{ y: 20, duration: 300 }}>
        <div class="profile-info">
          <div class="profile-avatar">
            {#if profile.avatar_url}
              <img src={profile.avatar_url} alt={profile.name} class="avatar-image" />
            {:else}
        ...
        {#if isOwnProfile}
          <div class="profile-actions">
            <a href="/settings/organizer-profile" use:link class="edit-profile-btn">
