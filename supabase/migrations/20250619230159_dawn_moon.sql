/*
  # Add avatar_url to organizers table

  1. Changes
    - Add `avatar_url` column to `organizers` table
    - Update `piece_details` view to use organizer's avatar_url directly
...
-- Drop and recreate the piece_details view to use organizer's avatar_url
...
SELECT
  ...
  o.avatar_url as organizer_avatar_url, -- Use organizer's avatar_url directly
  COALESCE(
    json_agg(
