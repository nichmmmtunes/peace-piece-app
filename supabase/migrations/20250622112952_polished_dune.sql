/*
  # Add artist_fees column and remove compensation_details

  1. Changes
    - Add `artist_fees` column to `pieces` table
...
-- First drop the view that depends on the pieces table
DROP VIEW IF EXISTS piece_details;

-- Add artist_fees column to pieces table
ALTER TABLE pieces
ADD COLUMN IF NOT EXISTS artist_fees bigint;

-- Recreate the piece_details view to include the artist_fees column
