/*
  # Add location and bio to profiles table

  1. Changes
    - Add `location` (text) column to profiles table
    - Add `bio` (text) column to profiles table with a CHECK constraint to limit to 250 characters
    - Add `followers_count` (integer) column to profiles table with default 0
    
  2. Security
    - Maintain existing RLS policies
*/

-- Add location and bio columns to profiles table
ALTER TABLE profiles 
ADD COLUMN IF NOT EXISTS location text,
ADD COLUMN IF NOT EXISTS bio text,
ADD COLUMN IF NOT EXISTS followers_count integer DEFAULT 0;

-- Add CHECK constraint to limit bio to 250 characters
ALTER TABLE profiles 
ADD CONSTRAINT bio_length_check CHECK (bio IS NULL OR LENGTH(bio) <= 250);
